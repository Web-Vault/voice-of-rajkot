<template>
  <div class="auth-bg min-h-screen w-full flex items-center justify-center relative overflow-hidden">
    ...
    <!-- Centered Card Form -->
    <div class="themed-card-form w-full max-w-lg relative z-10">
      <div class="px-8 py-10 md:px-10 md:py-12">
        <h2 class="text-2xl font-bold mb-2 text-indigo-800 text-center">Email Verification</h2>
        <p class="mb-8 text-gray-500 text-base text-center">Enter the OTP sent to your email to verify your account.</p>
        <form @submit.prevent="handleVerify" class="space-y-6">
          <div>
            <label for="otp" class="themed-label">OTP</label>
            <input id="otp" v-model="otp" type="text" class="themed-input" :class="{'input-error': error}" required />
            <transition name="fade"><span v-if="error" class="error-msg">{{ error }}</span></transition>
          </div>
          <button class="themed-btn w-full mt-2">Verify Email</button>
        </form>
        <div class="flex justify-between mt-8 text-sm">
          ...
          <a href="#" class="text-indigo-600 hover:underline" @click.prevent="handleResend">Resend OTP</a>
        </div>
      </div>
    </div>
  </div>
