  ...
  <div class="auth-split-bg min-h-screen flex">
    ...
    <div class="auth-split-left hidden md:flex flex-col justify-center items-center w-1/2 bg-gradient-to-br from-indigo-700 via-indigo-500 to-blue-400 text-white p-12 relative overflow-hidden">
      ...
      <div class="flex flex-col items-center z-10 relative">
        ...
        <p class="text-lg font-medium mb-8 opacity-90">Join the community. Share your art.</p>
      </div>
    ...
    <div class="auth-split-right flex flex-col justify-center items-center w-full md:w-1/2 bg-white px-6 py-12 md:px-16 min-h-screen">
      <div class="themed-card-form w-full max-w-lg relative">
        <div class="px-8 py-10 md:px-10 md:py-12">
          <h2 class="text-2xl font-bold mb-2 text-indigo-800 text-center">Create your account</h2>
          <p class="mb-8 text-gray-500 text-base text-center">Sign up to get started and join the artists of Rajkot.</p>
          <form @submit.prevent="handleRegister" class="space-y-6">
            <div>
              <label for="name" class="themed-label">Name</label>
              <input id="name" v-model="name" type="text" class="themed-input" :class="{'input-error': errors.name}" required />
              <transition name="fade"><span v-if="errors.name" class="error-msg">{{ errors.name }}</span></transition>
            </div>
            <div>
            ...
            <div class="themed-row-2col">
              ...
              <div>
                <label for="confirmPassword" class="themed-label">Confirm Password</label>
                <input id="confirmPassword" v-model="confirmPassword" type="password" class="themed-input" :class="{'input-error': errors.confirmPassword}" required />
                <transition name="fade"><span v-if="errors.confirmPassword" class="error-msg">{{ errors.confirmPassword }}</span></transition>
              </div>
            </div>
            <div class="flex items-center mb-2 mt-2">
              <input v-model="isArtist" type="checkbox" id="artist" class="mr-2" />
              <label for="artist" class="text-gray-700">Register as Artist/Poet</label>
            </div>
            <transition name="fade"><span v-if="errors.general" class="error-msg block mb-2 text-center">{{ errors.general }}</span></transition>
            <button class="themed-btn w-full mt-2">Register</button>
          </form>
          <div class="mt-8 text-sm text-center">
            <router-link to="/login" class="text-indigo-600 hover:underline">Already have an account? Login</router-link>
          </div>
        </div>
      </div>
    </div>
...
const errors = ref({ name: '', email: '', password: '', confirmPassword: '', general: '' });
